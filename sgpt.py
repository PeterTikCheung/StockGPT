...
tScores = []
apiCost = 0
client = OpenAI(api_key=open('auth.txt', 'r').readlines()[0].strip())
if not os.path.isdir('Individual_Reports'):
    os.mkdir('Individual_Reports')

def askGPT(prompt):
    global apiCost
    resp = client.chat.completions.create(model=modelV, temperature=args.temp, messages=[{'role': 'system', 'content': sysPrompt},{'role': 'user', 'content': prompt}])
...
for company in open('companies.txt', 'r').readlines():
    ...
    r = DDGS().news(keywords=company, safesearch='Off', timelimit='d')
    if r is None:
